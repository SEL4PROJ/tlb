theory Address_Translate_Tag_Ref

imports  MMU_Instants_TLB_Tag
...
lemma mmu_eq_asid_root_mem:
  "\<lbrakk> mmu_translate va (s::'a non_det_tlb_state_scheme) = (pa , s') \<rbrakk>  \<Longrightarrow> non_det_asid s =  non_det_asid s' \<and> TTBR0 s = TTBR0 s' \<and>  MEM s = MEM s'"
  by (clarsimp simp: mmu_translate_non_det_tlb_state_ext_def Let_def raise'exception_def  split: lookup_type.splits if_split_asm)

lemma mmu_det_eq_asid_root_mem:
  "\<lbrakk> mmu_translate va (s::'a det_tlb_state_scheme) = (pa , s') \<rbrakk>  \<Longrightarrow> det_asid s =  det_asid s' \<and> TTBR0 s = TTBR0 s' \<and>  MEM s = MEM s'"
  by (clarsimp simp: mmu_translate_det_tlb_state_ext_def Let_def raise'exception_def  split: lookup_type.splits if_split_asm)
 
lemma mmu_sat_eq_asid_root_mem:
  "\<lbrakk> mmu_translate va (s::'a sat_tlb_state_scheme) = (pa , s') \<rbrakk>  \<Longrightarrow> sat_asid s =  sat_asid s' \<and> TTBR0 s = TTBR0 s' \<and> MEM s = MEM s'"
  by (clarsimp simp: mmu_translate_sat_tlb_state_ext_def Let_def  raise'exception_def split:lookup_type.splits if_split_asm)


lemma mmu_incon_eq_asid_root_mem':
  "\<lbrakk> mmu_translate va (s::'a set_tlb_state_scheme) = (pa , s') \<rbrakk>  \<Longrightarrow> set_asid s =  set_asid s' \<and> TTBR0 s = TTBR0 s' \<and> MEM s = MEM s'"
...
lemma mmu_translate_sat_tlb_union:
  "mmu_translate v s = (p,t) \<Longrightarrow> 
      sat_tlb t = sat_tlb s \<union> the ` {e\<in>pt_walk (sat_asid s)  (MEM s) (TTBR0 s) ` UNIV. \<not>is_fault e}"
  by (clarsimp simp:  mmu_translate_sat_tlb_state_ext_def Let_def raise'exception_def split:lookup_type.splits if_split_asm)
...
lemma  mmu_translate_non_det_det_refine:
  ...
  apply (subgoal_tac "lookup'' (non_det_tlb s - tlb_evict (typ_non_det_tlb s)) (non_det_asid s) va \<le> lookup'' (det_tlb t) (det_asid t) va")
   prefer 2 apply (simp add: asid_tlb_mono)
  ...
  apply (cases "lookup'' (det_tlb t) (det_asid t) va" ; clarsimp simp: Let_def)
    ...
    apply (rule conjI) 
     ...
     apply (simp add: is_fault_def asid_tlb_lookup_refill)
    apply (cases s, cases t, clarsimp simp: state.defs)
    apply fastforce
  ...
  apply (cases "lookup'' (non_det_tlb s - tlb_evict (typ_non_det_tlb s)) (non_det_asid s)  va"; clarsimp)
   apply (clarsimp simp : consistent0_def Let_def tlb_rel_det_def 
      lookup_in_asid_tlb split: if_split_asm)
   apply (drule lookup_in_asid_tlb)
  ...
  using lookup_in_asid_tlb by blast
...
lemma mmu_translate_det_sat_refine:
  ...
  apply (subgoal_tac "lookup'' (det_tlb s) (det_asid s) va \<le> lookup'' (sat_tlb t \<union> the `{e\<in>pt_walk (sat_asid t)  (MEM s) (TTBR0 s) ` UNIV. \<not> is_fault e}) (sat_asid t) va")
   prefer 2
   apply (simp add: saturated_def sup.absorb1 asid_tlb_mono tlb_rel_sat_def)
  apply (subgoal_tac "sat_tlb t = sat_tlb t \<union> the `{e\<in>pt_walk (sat_asid t)  (MEM s) (TTBR0 s) ` UNIV. \<not> is_fault e}")
   prefer 2
  ...
  apply (cases "lookup'' (sat_tlb t \<union> the `{e \<in> range (pt_walk (sat_asid t) (MEM s) (TTBR0 s)). \<not> is_fault e}) (sat_asid t) va"; clarsimp simp: Let_def)
    apply (clarsimp simp: tlb_rel_sat_def Let_def)
    apply (subgoal_tac "is_fault (pt_walk (sat_asid t)  (MEM s) (TTBR0 s) va)")
     apply (clarsimp simp: raise'exception_def  saturated_def state.defs  split: if_split_asm) 
    apply (clarsimp simp: lookup_def tagged_entry_set_def entry_set_def is_fault_def split: if_split_asm) 
    (* force not working anymore because to tagged_entry_set *)
    apply (clarsimp simp: asid_entry_pt_walk asid_va_entry_range_pt_entry consistent0_def is_fault_def lookup_in_asid_tlb not_Some_eq option.sel sat_miss_fault)
    apply safe [1]
       apply (metis (full_types, hide_lams) asid_entry_pt_walk_no_fault is_fault_def lookup_asid_tlb_hit_entry_range lookup_in_asid_tlb option.sel option.simps(3))
      apply (metis (full_types, hide_lams) asid_entry_pt_walk_no_fault is_fault_def lookup_asid_tlb_hit_entry_range lookup_in_asid_tlb option.sel option.simps(3))
     apply (metis (no_types, hide_lams) le_iff_sup lookup_asid_tlb_miss_union_hit_intro lookup_type.simps(5) sup.commute)
  using is_fault_def sat_miss_fault apply force
    (* until here *)
   apply (clarsimp simp: consistent0_def)
  apply (subgoal_tac "x3 = the (pt_walk (sat_asid t) (MEM s) (TTBR0 s) va)")
   prefer 2
   apply (clarsimp simp: consistent0_def)
  apply (cases "lookup'' (det_tlb s) (sat_asid t) va"; clarsimp simp: Let_def)
  apply (subgoal_tac "\<not>is_fault (pt_walk (sat_asid t) (MEM s) (TTBR0 s) va)")
   prefer 2
   apply (clarsimp simp: tlb_rel_sat_def is_fault_def lookup_in_asid_tlb) 
   apply (subgoal_tac "\<not>is_fault (pt_walk (sat_asid t)  (MEM s) (TTBR0 s) va)")
    apply (clarsimp simp: tlb_rel_sat_def is_fault_def lookup_in_asid_tlb)
   apply (simp add: consistent_not_Incon_imp is_fault_def)
  by (clarsimp simp: tlb_rel_sat_def typ_sat_tlb_def typ_det_tlb_def state.defs lookup_in_asid_tlb raise'exception_def split: if_split_asm)
...
lemma mmu_translate_sat_consistent:
  ...
  apply (subgoal_tac "sat_tlb s = sat_tlb s \<union> the ` {e\<in>pt_walk (sat_asid s) (MEM s) (TTBR0 s) ` UNIV. \<not>is_fault e}")
   prefer 2
...
lemma mmu_translate_sat_incon_refine_pa:
  ...
  apply (subgoal_tac "sat_tlb s = sat_tlb s \<union> the ` {e\<in>pt_walk (sat_asid s) (MEM s) (TTBR0 s) ` UNIV. \<not>is_fault e}")
   prefer 2
   apply (fastforce simp: saturated_def)
  apply (cases "lookup'' (sat_tlb s \<union> the ` {e\<in>pt_walk (sat_asid s) (MEM s) (TTBR0 s) ` UNIV. \<not>is_fault e}) (sat_asid s)  va")
    apply clarsimp
    apply (frule_tac va = va in  sat_miss_fault, simp) 
    apply (clarsimp simp: raise'exception_def is_fault_def split:if_split_asm)
   apply (clarsimp simp: consistent0_def)
  apply (subgoal_tac "x3 = the (pt_walk (sat_asid s) (MEM s) (TTBR0 s) va)")
   prefer 2
   apply (clarsimp simp: consistent0_def)
  apply (subgoal_tac "\<not>is_fault (pt_walk (sat_asid s) (MEM s) (TTBR0 s) va)", clarsimp)
  by (clarsimp simp: is_fault_def lookup_in_asid_tlb consistent0_def)
...
lemma mmu_translate_sat_incon_refine_tlb_rel:
  ...
  apply (subgoal_tac "s' = s\<lparr>exception := exception s'\<rparr> \<and> t' = t\<lparr>exception := exception t'\<rparr>")
   ...
   apply (metis (no_types, lifting) Un_def fst_def mmu_translate_sat_tlb_union prod_eqI saturated_tlb)
  apply (subgoal_tac "sat_tlb s' = sat_tlb s \<union> the ` {e\<in>pt_walk (sat_asid s) (MEM s) (TTBR0 s) ` UNIV. \<not>is_fault e}  \<and> 
   ...
   apply (cases "lookup'' (sat_tlb s \<union> the ` {e\<in>pt_walk (sat_asid s) (MEM s) (TTBR0 s) ` UNIV. \<not>is_fault e}) (sat_asid s) va")
    ...
    apply (subgoal_tac "sat_tlb s  = sat_tlb s \<union> the ` {e \<in> range (pt_walk (sat_asid s) (MEM s) (TTBR0 s)). \<not> is_fault e}") 
     apply (clarsimp simp: consistent0_def) 
    apply (clarsimp simp: saturated_def) apply force 
   apply (subgoal_tac "x3 = the (pt_walk (sat_asid s) (MEM s) (TTBR0 s) va)")
    prefer 2
    apply (clarsimp simp: consistent0_def)
   apply (subgoal_tac "\<not>is_fault (pt_walk (sat_asid s)  (MEM s) (TTBR0 s) va)")
    prefer 2
    apply (clarsimp simp: consistent0_def is_fault_def lookup_in_asid_tlb)
   apply (simp only: mmu_translate_set_tlb_state_ext_def Let_def is_fault_def)  apply (cases t, cases s, cases t', cases s', clarsimp)
  by (simp only: mmu_translate_sat_tlb_union mmu_sat_eq_asid_root_mem is_fault_def) 
...
lemma mmu_translate_sat_incon_refine_consistent:
  ...
  apply (subgoal_tac "\<not>is_fault (pt_walk (sat_asid s) (MEM s) (TTBR0 s) va)")
   prefer 2
   apply (clarsimp simp: is_fault_def lookup_in_asid_tlb consistent0_def)
  apply clarsimp
  done
...
lemma mmu_translate_sat_incon_refine:
  "\<lbrakk> mmu_translate va s = (pa, s');  mmu_translate va t = (pa', t') ;
           va \<notin> iset (set_tlb t) ; tlb_rel_abs (typ_sat_tlb s) (typ_set_tlb t) \<rbrakk> \<Longrightarrow> 
                  pa = pa' \<and>  tlb_rel_abs  (typ_sat_tlb s') (typ_set_tlb t')  \<and>  va \<notin> iset (set_tlb t')"

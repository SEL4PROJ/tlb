...
define TLBMaintain > ITLBIMVA
   ( rt :: word )
   =
   {
    var mva  = rt<31:12>;
    for i in 0 .. microInstrTLBEntries - 1 do 
        when mva_entry_match (InstrTLB([i]:: bits(6)), mva) do InstrTLB([i]:: bits(6)) <- None;
    IncPC()
   }

define TLBMaintain > ITLBIASID
   ( rt :: word )
   =
   {
    var asid = rt<7:0>;
    ...
    {
     case None => nothing
     case Some (e) =>  when tag_entry (e) == Some (asid) do
           InstrTLB([i]:: bits(6)) <- None  
       };
    IncPC()
   }

define TLBMaintain > DTLBIALL
   ( rt :: word )
   =
   {
...
define TLBMaintain > DTLBIMVA
   ( rt :: word )
   =
   {
    var mva  = rt<31:12>;
    for i in 0 .. microDataTLBEntries - 1 do 
        when mva_entry_match (DataTLB([i]:: bits(5)), mva) do DataTLB([i]:: bits(5)) <- None;
    IncPC()
   }


define TLBMaintain > DTLBIASID
   ( rt :: word )
   =
   {
    var asid = rt<7:0>;
...
define TLBMaintain > TLBIMVA
   ...
   =
   {
    ...
    for i in 0 .. microDataTLBEntries - 1 do 
        when mva_entry_match (DataTLB([i]:: bits(5)), mva) do DataTLB([i]:: bits(5)) <- None;
    for i in 0 .. mainTLBEntries - 1 do 
        when mva_entry_match (unified_mainTLB([i]:: bits(8)), mva) do unified_mainTLB([i]:: bits(8)) <- None;
    IncPC()
   }


define TLBMaintain > TLBIASID
   ( rt :: word )
   =
   {
    var asid = rt<7:0>;

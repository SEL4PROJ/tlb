...
definition
  ptrace_set ::"32 word set \<Rightarrow> p_state \<Rightarrow> paddr set"
where
  "ptrace_set V s = \<Union>(ptable_trace' (heap s) (root s) ` (Addr ` V)) "
...
lemma cons_set_preserved:
  "\<lbrakk> \<forall>va\<in>SM. \<exists>p.  ptable_lift_m (heap s) (root s) (mode s) (Addr va) = Some p \<and> p \<notin> ptrace_set SM s;
          \<forall>va\<in>SM. (asid s, va) \<notin> incon_set s; p \<notin> ptrace_set SM s; va \<in> SM\<rbrakk>  \<Longrightarrow> 
              (asid s, va) \<notin> pde_comp' (asid s) (heap s) (heap s(p \<mapsto> v)) (root s)"
  apply (case_tac "mode s")
   ...
   apply (drule_tac x = va in bspec , simp)
   apply (clarsimp simp: ptable_trace'_def  Let_def lookup_pde'_def  get_pde'_def decode_heap_pde'_def split: option.splits pde.splits pte.splits)
...
lemma safe_set_preserved':
  " \<Turnstile> \<lbrace>\<lambda>s. safe_set SM s \<and> (\<exists>vp v. aval lval s = Some vp \<and> aval rval s = Some v \<and> vp \<in> SM \<and>
     Q (s\<lparr>heap := heap s(the ( ptable_lift_m (heap s) (root s) (mode s) (Addr vp)) \<mapsto> v), 
            incon_set := incon_set s \<union> pde_comp (asid s) (heap s) (heap s(the ( ptable_lift_m (heap s) (root s) (mode s) (Addr vp)) \<mapsto> v)) (root s)\<rparr>)) \<rbrace>
                       lval ::= rval \<lbrace>\<lambda>s. safe_set SM s \<rbrace>"
  ...
  "\<Turnstile> \<lbrace> \<lambda>s. safe_set SM s \<and> (\<exists>vp v. aval lval s = Some vp \<and> aval rval s = Some v \<and> vp \<in> SM \<and> 
     Q (s \<lparr>heap := heap s (the ( ptable_lift_m (heap s) (root s) (mode s) (Addr vp)) \<mapsto> v),
             incon_set := incon_set s \<union> pde_comp' (asid s) (heap s) (heap s(the (ptable_lift_m (heap s)
                           (root s) (mode s) (Addr vp)) \<mapsto> v)) (root s)\<rparr>)) \<rbrace>
...
lemma weak_pre_write'_user:
  ...
  apply (subgoal_tac "incon_set s = incon_set s \<union> pde_comp' (asid s) (heap s) (heap s(the (ptable_lift_m (heap s) (root s) (mode s) (Addr vp)) \<mapsto> v)) (root s)")
   apply (clarsimp simp: safe_set_def con_set_def safe_memory_def)
...
lemma weak_pre_write'':
  "\<Turnstile> \<lbrace> \<lambda>s. aval lval s = Some vp \<and> aval rval s = Some v \<and> vp \<in> SM \<and> safe_set SM s \<and> 
...
(* safe set in the context switch, not being used
    in the example *)
definition
  SM_user :: "heap \<Rightarrow> paddr \<Rightarrow> 32 word set"
where

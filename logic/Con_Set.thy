...
(* vcgm check for the flush and con_set memory writes, in Kernel Mode  *)
...
lemma [simp]:
  " mem_read_hp' (asid sa) (incon_set sa) (heap sa) (root sa) (Addr x2) = Some v
                \<Longrightarrow> mem_read_hp' (asid sa) {av \<in> incon_set sa. fst av \<noteq> s \<and> snd av \<noteq> vp} (heap sa) (root sa) (Addr x2) = Some v"

...
lemma [simp]:
  " incon_set s - ({asid s} \<times> UNIV) = {av \<in> incon_set s. fst av \<noteq> asid s} "
...
lemma [simp]:
  "mem_read_hp' (asid sa) (incon_set sa) (heap sa) (root sa) (mode sa) (Addr x2) = Some v
                \<Longrightarrow> mem_read_hp' (asid sa) (incon_set sa - {(asid s, vp)}) (heap sa) (root sa) (mode sa) (Addr x2) = Some v"
  by (clarsimp simp: mem_read_hp'_def split: if_split_asm)

lemma  [simp]:
  "aval e sa = Some v \<Longrightarrow>  aval e (sa\<lparr>incon_set := incon_set sa - {(asid s, vp)}\<rparr>) = Some v"
  by(induct e arbitrary: v sa; clarsimp split: option.splits)


lemma  [simp]:
  "aval e sa = Some v \<Longrightarrow>  aval e (sa\<lparr>incon_set :=  incon_set sa - {(asid s, vp)}, 
...
lemma flush_ASID_va_mem_write:
  "\<Turnstile> \<lbrace> \<lambda>s. aval lval s = Some vp \<and> aval rval s = Some v \<and>  asid s = a \<and> (asid s, Addr vp) \<in> incon_set s \<and> mode s = Kernel \<and> 
      ptable_lift_m (heap s) (root s) (mode s) (Addr vp) = Some pp \<rbrace>  Flush (flushASIDvarange a {Addr vp}) ;; lval ::= rval \<lbrace>\<lambda>s. heap s pp = Some v \<rbrace>"
  by vcgm

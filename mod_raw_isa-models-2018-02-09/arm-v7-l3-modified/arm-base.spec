...
unit DataAbort (vaddress :: bits(32), ipaddress ::  bits(40), domain :: bits(4), level :: int, 
 ...
 TakeDataAbortException()  -- using an exiteting l3 function
...
AddressDescriptor * AddressDescriptor * bits(32) * bits(4)
      level1_desc_address_and_desc (n :: nat, ttbr :: bits(32), mva :: bits(32)) =
 {
  ...
  if n==0 then  l1descaddr.paddress.physicaladdress <-  ttbr<31:(14-0)> : mva<(31-0):20> : '00'
  else if n == 1 then l1descaddr.paddress.physicaladdress <-  ttbr<31:(14-1)> : mva<(31-1):20> : '00'
  else if n == 2 then l1descaddr.paddress.physicaladdress <-  ttbr<31:(14-2)> : mva<(31-2):20> : '00'
  else if n == 3 then l1descaddr.paddress.physicaladdress <-  ttbr<31:(14-3)> : mva<(31-3):20> : '00'
  else if n == 4 then l1descaddr.paddress.physicaladdress <-  ttbr<31:(14-4)> : mva<(31-4):20> : '00'
  else if n == 5 then l1descaddr.paddress.physicaladdress <-  ttbr<31:(14-5)> : mva<(31-5):20> : '00'
  else if n == 6 then l1descaddr.paddress.physicaladdress <-  ttbr<31:(14-6)> : mva<(31-6):20> : '00'
  else l1descaddr.paddress.physicaladdress <-  ttbr<31:(14-7)> : mva<(31-7):20> : '00';
  ...
  return (l1descaddr, l1descaddr2, l1desc, hintsattrs)
}
...
AddressDescriptor * AddressDescriptor * bits(32)
      level2_desc_address_and_desc (l1desc :: bits(32), mva :: bits(32), l1descaddr :: AddressDescriptor) =
 {
   ...
   l2descaddr.paddress.physicaladdress <-  l1desc<31:10> : mva<19:12> : 0 :: bits(2); 
   ...
   l2descaddr.memattrs <- l1descaddr.memattrs;
     ...
     return (l2descaddr, l2descaddr2, l2desc )
}
       ...
       physicaladdressi :: bits (32), mva :: bits (32)) = 
{
 ...
 -- Decode the TEX, C, B and S bits to produce the TLBRecord's memory attributes 
  ...
  result.addrdesc.paddress.physicaladdress <- physicaladdressi; 
...
TLBRecord TranslationTableWalkSD (mva :: bits(32), is_write :: bool, size  :: nat) = 
{
 ...
 -- variables for the remaining
  ...
  when HaveSecurityExt() and disabled do
   {
    level <- 1;
    DataAbort(mva, IA, domain, 1, is_write, DAbort_Translation, 
    taketohypmode, stage2, ipavalid, LDFSRformat, s2fs1walk)
   };

  -- Obtain First level descriptor
  l1descaddr, l1descaddr2, l1desc, hintsattrs   =  level1_desc_address_and_desc (n, ttbr, mva);
 ...
 -- Process First level descriptor 
      ...
      case 1 => -- Large page or Small page  
         {
          ...
          -- Obtain Second level descriptor. 
          l2descaddr , l2descaddr2, l2desc  = level2_desc_address_and_desc (l1desc, mva, l1descaddr);
          ...
          writing_access_flag (l2desc, l2descaddr2, mva, IA, domain, level, is_write, DAbort_AccessFlag, taketohypmode, 
                                 stage2, ipavalid, LDFSRformat, s2fs1walk );
  ...
  var result = TLBResult (texcb, S, ap , xn , pxn , 
...
-- add TLB before that
-- TranslateAddressV() 
AddressDescriptor * TLBRecord * TLBRecord TranslateAddressV (va :: bits(32), ispriv :: bool, iswrite :: bool,  size :: nat) =
{
  var mva         :: bits(32);
  --var ia_in       :: bits(40);
  var result      :: AddressDescriptor;
  ...
  if (ishyp and CP15.HSCTLR.M) or (!ishyp and CP15.VSCTLR.M) then 
   { -- Stage 1 MMU enabled  
      tlbrecordS1 <- TranslationTableWalkSD(mva, iswrite, size);
      when CheckDomain(tlbrecordS1.domain, mva, tlbrecordS1.level, iswrite) do
          CheckPermission(tlbrecordS1.perms, mva, tlbrecordS1.level, tlbrecordS1.domain,iswrite, ispriv, ishyp, false) -- making usesLD false 
   }
  else
    tlbrecordS1 <- TranslateAddressVS1Off(mva);

 -- we do not support second stage translation
  return (result , tlbrecordS1, tlbrecordS2)
 }

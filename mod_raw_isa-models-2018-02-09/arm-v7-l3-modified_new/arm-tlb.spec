     ...
     case 4    =>  --small page
      {
       ...
       e1.asid           <- a;
       e1.vadSm          <- va<31:12>;
       e1.padSm          <- e.addrdesc.paddress.physicaladdress<31:12>;
     ...
     case 64   =>  --large page
      {
       ...
       e1.asid           <- a;
       e1.vadLr          <- va<31:16>;
       e1.padLr          <- e.addrdesc.paddress.physicaladdress<31:16>;
       -- e1.NS             <- e.addrdesc.paddress.NS;
       e1.memattrs       <- e.addrdesc.memattrs;
       e1.perms          <- e.perms;
       e1.nG             <- e.nG;
     ...
     case 1024 =>  -- section
      {
       ...
       e1.asid            <- a;
       e1.vadSec          <- va<31:20>;
       e1.padSec          <- e.addrdesc.paddress.physicaladdress<31:20>;
       -- e1.NS              <- e.addrdesc.paddress.NS;
       e1.memattrs        <- e.addrdesc.memattrs;
       e1.perms           <- e.perms;
       e1.nG              <- e.nG;
     ...
     case _    =>  -- supersection
      {
       ...
       e1.asid            <- a;
       e1.vadSup          <- va<31:24>;
       e1.padSup          <- e.addrdesc.paddress.physicaladdress<31:24>;
       -- e1.NS             <- e.addrdesc.paddress.NS;
       e1.memattrs        <- e.addrdesc.memattrs;
       e1.perms           <- e.perms;
       e1.nG              <- e.nG;
...
AddressDescriptor va_to_pa  (v :: bits(32), e :: TLBEntry) = 
  {
   var adresdec :: AddressDescriptor;
    match e 
    {
 	   case EntrySmall (sme) => 
      {
        adresdec.paddress.physicaladdress <-  sme.padSm  : v<11:0>;
        --adresdec.paddress.NS              <- sme.NS;
        adresdec.memattrs                 <- sme.memattrs
       }  
 	   case EntryLarge   (lre) => 
      {
        adresdec.paddress.physicaladdress <- lre.padLr  : v<15:0>;
      ...
      {
        adresdec.paddress.physicaladdress <- sce.padSec : v<19:0>;
        --adresdec.paddress.NS              <- sce.NS;
        adresdec.memattrs                 <- sce.memattrs
       }
 	   case EntrySuper (spe) => 
      {
        adresdec.paddress.physicaladdress <- spe.padSup : v<23:0>;
...
unit Configure_CortexA9 = 
{
  -- Architecture
  Architecture <- ARMv7_A;

  -- disable virtual extensions, cortex A9 does not support it
  ...
  -- disable hypervisor
  when CPSR.M == 0b11010 do #IMPLEMENTATION_DEFINED "Hypervisor not supported";
  ...
  CP15.TTBCR.EAE <- false;
   ...
   -- Fast Context Switch Extensions: in the Cortex-A9 processor, VA and MVA are identical. CP15.FCSEIDR.PID should be zero so that FCSETranslate ensures mva = va in  TranslateAddressV
  CP15.FCSEIDR.PID <- 0
}
         ...
         match lookupTLB_main (asid, address)
			    {
				   case Miss => 
				    {
             -- TranslationTableWalkSD checks whether translation using page table is enabled or not, it also does CheckPermission and CheckDomain
					   var memaddrdesc = Fst(TranslateAddressV(address, privileged, iswrite, size));
             var tlb_entry = Fst(Snd(TranslateAddressV(address, privileged, iswrite, size)));
						 -- replacement

...
declare MEM :: bits(40) -> byte

bool list mem1 (address::bits(40)) = [MEM(address)]


-- B2.4.2 Basic memory accesses
-- have to see NS bit here, _Mem[] in the manual

component mem (memaddrdesc::AddressDescriptor, size::nat) :: bool list
{  value =
      match size
      { case 1 => (mem1(memaddrdesc.paddress.physicaladdress + 0))<7:0>
        case 2 => (mem1(memaddrdesc.paddress.physicaladdress + 1) : mem1(memaddrdesc.paddress.physicaladdress + 0))<15:0>
        case 4 => (mem1(memaddrdesc.paddress.physicaladdress + 3) : mem1(memaddrdesc.paddress.physicaladdress + 2) :
                   mem1(memaddrdesc.paddress.physicaladdress + 1) : mem1(memaddrdesc.paddress.physicaladdress + 0))<31:0>
        case 8 => (mem1(memaddrdesc.paddress.physicaladdress + 7) : mem1(memaddrdesc.paddress.physicaladdress + 6) :
                   mem1(memaddrdesc.paddress.physicaladdress + 5) : mem1(memaddrdesc.paddress.physicaladdress + 4) :
                   mem1(memaddrdesc.paddress.physicaladdress + 3) : mem1(memaddrdesc.paddress.physicaladdress + 2) :
                   mem1(memaddrdesc.paddress.physicaladdress + 1) : mem1(memaddrdesc.paddress.physicaladdress + 0))<63:0>
        case _ => #ASSERT("mem: size in {1, 2, 4, 8}")
      }
   assign value =
      match size
      { case 1 =>   MEM(memaddrdesc.paddress.physicaladdress + 0) <- [value<7:0>]
        case 2 => { MEM(memaddrdesc.paddress.physicaladdress + 0) <- [value<7:0>];
                    MEM(memaddrdesc.paddress.physicaladdress + 1) <- [value<15:8>]
                  }
        case 4 => { MEM(memaddrdesc.paddress.physicaladdress + 0) <- [value<7:0>];
                    MEM(memaddrdesc.paddress.physicaladdress + 1) <- [value<15:8>];
                    MEM(memaddrdesc.paddress.physicaladdress + 2) <- [value<23:16>];
                    MEM(memaddrdesc.paddress.physicaladdress + 3) <- [value<31:24>]
                  }
        case 8 => { MEM(memaddrdesc.paddress.physicaladdress + 0) <- [value<7:0>];
                    MEM(memaddrdesc.paddress.physicaladdress + 1) <- [value<15:8>];
                    MEM(memaddrdesc.paddress.physicaladdress + 2) <- [value<23:16>];
                    MEM(memaddrdesc.paddress.physicaladdress + 3) <- [value<31:24>];
                    MEM(memaddrdesc.paddress.physicaladdress + 4) <- [value<39:32>];
                    MEM(memaddrdesc.paddress.physicaladdress + 5) <- [value<47:40>];
                    MEM(memaddrdesc.paddress.physicaladdress + 6) <- [value<55:48>];
                    MEM(memaddrdesc.paddress.physicaladdress + 7) <- [value<63:56>]
                  }
        case _ => #ASSERT("mem: size in {1, 2, 4, 8}")
      }
}
...
bool TLBLookupCameFromCacheMaintenance() = UNKNOWN

bool HaveLPAE () = UNKNOWN

-- Data Abort types.
construct DAbort {
    DAbort_AccessFlag  DAbort_Alignment DAbort_Background  DAbort_Domain  DAbort_Permission 
    DAbort_Translation DAbort_SyncExternal DAbort_SyncExternalonWalk DAbort_SyncParity DAbort_SyncParityonWalk 
    DAbort_AsyncParity DAbort_AsyncExternal DAbort_DebugEvent DAbort_TLBConflict DAbort_Lockdown DAbort_Coproc DAbort_ICacheMaint}
...
bits(9) LSInstructionSyndrome() =  return UNKNOWN
    ...
    if !taketohypmode then
     {
      ...
      if !(dabtype == DAbort_AsyncParity or dabtype == DAbort_AsyncExternal or dabtype == DAbort_DebugEvent) then
          CP15.&DFAR <- vaddress
      else 
      ...
      if LDFSRformat then -- new format
       {
        ...
        if (dabtype == DAbort_AsyncExternal or dabtype == DAbort_SyncExternal) then
      ...
      else
       {
        ...
        var domain_valid = ((dabtype == DAbort_Domain) or ((level == 2) and 
                        (dabtype == DAbort_Translation or dabtype == DAbort_AccessFlag or 
                         dabtype == DAbort_SyncExternalonWalk or dabtype == DAbort_SyncParityonWalk ) ) or 
     ...
     nothing;  -- PMSA not supported
 --{   CP15.&DFSR <- UNKNOWN;
 --   CP15.&DFSR <- bits(32) UNKNOWN;
 --   if !(type IN {DAbort_AsyncParity,DAbort_AsyncExternal,
 --                 DAbort_DebugEvent,DAbort_SyncParity}) 
 --   then DFAR = vaddress;
 --   else if type == DAbort_SyncParity then 
 --            DFAR = IMPLEMENTATION_DEFINED
 --        else when type == DAbort_DebugEvent do -- Watchpoint
...
unit CheckPermission (perms :: Permissions, mva :: bits(32), level :: int, domain :: bits(4), iswrite :: bool, 
                      ispriv :: bool, taketohypmode :: bool, LDFSRformat :: bool) =
 {
  ...
  match ap_perms
  {
   ...
   case '111' =>
       if MemorySystemArchitecture() == MemArch_VMSA then abort <- iswrite else #UNPREDICTABLE("access fault")
  };
...
bool IsBVZero (bv :: bits(n) ) = if bv == (0`n) then true else false
...
AddressDescriptor SecondStageTranslate (s1outaddrdesc :: AddressDescriptor, mva ::  bits(32) ) =
{
  ...
  if HaveVirtExt() and !IsSecure() and !CurrentModeIsHyp() then 
  {
    ...
    CheckPermissionS2 (tlbrecordS2.perms, mva, s2ia, tlbrecordS2.level, is_write, s2fs1walk);
      ...
      when tlbrecordS2.addrdesc.memattrs.MemType != MemType_Normal do
        {
         ...
         LDFSRformat <- true;
         s2fs1walk <- true;
         DataAbort (mva, s2ia, domain, tlbrecordS2.level, is_write, DAbort_Permission, taketohypmode,
                     secondstageabort, ipavalid, LDFSRformat, s2fs1walk)
          }
        }; 
...
bool RemapRegsHaveResetValues() = UNKNOWN
...
TLBRecord TranslationTableWalkSD (mva :: bits(32), is_write :: bool, size  :: nat) = 
{
 --  this is only called when the MMU is enabled 
  ...
  var l1descaddr :: AddressDescriptor; 
  var l2descaddr :: AddressDescriptor;
 ...
 -- default setting of the domain 
  ...
  var n = [CP15.TTBCR.N] :: nat;

 -- variables for the remaining
  var ttbr;
  var disabled;
  var level;
  var hintsattrs;
  ...
  var xn;
  var blocksize :: nat;
  var physicaladdressexti :: bits(8);
  var physicaladdressi :: bits(32);
  var l1descaddr2;
  var l2descaddr2;

  -- Determine correct Translation Table Base Register to use. 
  if  n == 0 or (n == 1 and IsBVZero([mva<31>] :: bits(1))) or 
     (n == 2 and IsBVZero(mva<31:30>)) or (n == 3 and IsBVZero(mva<31:29>)) or 
     (n == 4 and IsBVZero(mva<31:28>)) or (n == 5 and IsBVZero(mva<31:27>)) or 
     (n == 6 and IsBVZero(mva<31:26>)) or (n == 7 and IsBVZero(mva<31:25>))
  then
   {
    ttbr     <- CP15.&TTBR0;
    disabled <- CP15.TTBCR.PD0
    }
  else
   {
    ttbr     <- CP15.&TTBR1;
    disabled <- CP15.TTBCR.PD1;
    n        <- 0 -- TTBR1 translation always works like N=0 TTBR0 translation
    };

  -- Check this Translation Table Base Register is not disabled. 
  when HaveSecurityExt() and disabled do
   {
    level <- 1;
  ...
  if n==0 then  l1descaddr.paddress.physicaladdress <- '00000000' : ttbr<31:(14-0)> : mva<(31-0):20> : '00'
  else if n == 1 then l1descaddr.paddress.physicaladdress <- '00000000' : ttbr<31:(14-1)> : mva<(31-1):20> : '00'
  else if n == 2 then l1descaddr.paddress.physicaladdress <- '00000000' : ttbr<31:(14-2)> : mva<(31-2):20> : '00'
  else if n == 3 then l1descaddr.paddress.physicaladdress <- '00000000' : ttbr<31:(14-3)> : mva<(31-3):20> : '00'
  else if n == 4 then l1descaddr.paddress.physicaladdress <- '00000000' : ttbr<31:(14-4)> : mva<(31-4):20> : '00'
  else if n == 5 then l1descaddr.paddress.physicaladdress <- '00000000' : ttbr<31:(14-5)> : mva<(31-5):20> : '00'
  else if n == 6 then l1descaddr.paddress.physicaladdress <- '00000000' : ttbr<31:(14-6)> : mva<(31-6):20> : '00'
  else l1descaddr.paddress.physicaladdress <- '00000000' : ttbr<31:(14-7)> : mva<(31-7):20> : '00';
  ...
  when CP15.SCTLR.EE  do  l1desc <- [BigEndianReverse(mem(l1descaddr2, 4), 4)] :: bits(32);
 ...
 -- Process First level descriptor 
  match l1desc<1:0> 
   {
     case 0  =>    -- Fault, Reserved 
         {
          level <- 1;
          DataAbort(mva, IA, domain, level, is_write, DAbort_Translation, taketohypmode, stage2, ipavalid, LDFSRformat, s2fs1walk)
          }
      case 1 => -- Large page or Small page  
         {
          ...
          l2descaddr.paddress.physicaladdress <- 0:: bits(8) : l1desc<31:10> : mva<19:12> : 0 :: bits(2); 
          l2descaddr.paddress.NS <- [!IsSecure()] :: bits(1); 
          ...
          when CP15.SCTLR.EE do l2desc <- [BigEndianReverse(mem(l2descaddr2, 4),4)] :: bits(32);
 
          -- Process Second level descriptor. 
          when l2desc<1:0> == '00' do
            DataAbort(mva, IA, domain, level, is_write, DAbort_Translation, taketohypmode, 
                      stage2, ipavalid, LDFSRformat, s2fs1walk);
          S <- [l2desc<10>] :: bits(1); 
          ...
          when CP15.VSCTLR.AFE and !l2desc<4> do
           {   
            if !CP15.VSCTLR.HA  then
              DataAbort(mva, IA, domain, level, is_write, DAbort_AccessFlag, taketohypmode, 
                                 stage2, ipavalid, LDFSRformat, s2fs1walk)
      ...
      case _ => -- Section or Supersection 
       {
        ...
        when CP15.VSCTLR.AFE  and !l1desc<10> do
         {   
          if !CP15.VSCTLR.HA  then
           DataAbort(mva, IA, domain, level, is_write, DAbort_AccessFlag, taketohypmode, stage2, ipavalid, LDFSRformat, s2fs1walk)
          else -- Hardware-managed Access flag must be set in memory 
         ...
         if !l1desc<18> then -- Section
          ...
          { 
           domain <- '0000';
           blocksize <- 16384;
           physicaladdressexti <- l1desc<8:5> : l1desc<23:20>; 
  ...
  result.addrdesc.paddress.physicaladdress <- physicaladdressexti:physicaladdressi; 
 ...
 -- check for alignment issues if memory type is SO or Device 
  when (result.addrdesc.memattrs.MemType == MemType_Device or result.addrdesc.memattrs.MemType == MemType_StronglyOrdered) do 
     when mva != Align(mva, size) do AlignmentFaultV(mva, false, false); 
  return result
}
...
-- TranslateAddressV() 
AddressDescriptor TranslateAddressV (va :: bits(32), ispriv :: bool, iswrite :: bool,  size :: nat) =
{
  var mva         :: bits(32);
  var ia_in       :: bits(40);
  var result      :: AddressDescriptor;
  ...
  if (ishyp and CP15.HSCTLR.M) or (!ishyp and CP15.SCTLR.M) then 
   { -- Stage 1 MMU enabled
    var usesLD = ishyp or CP15.TTBCR.EAE;
    if usesLD then
     {
      ia_in <- '00000000':mva;
      tlbrecordS1 <- tlbrecordS1; -- TranslationTableWalkLD(ia_in, mva, iswrite, true, false, size); 
      CheckPermission(tlbrecordS1.perms, mva, tlbrecordS1.level, tlbrecordS1.domain, iswrite, ispriv, ishyp, usesLD)
      }
    else  
     { 
  ...
  if HaveVirtExt() and !IsSecure() and !ishyp then
    if CP15.HCR.VM  then -- second stage enabled
     {
      var s1outputaddr = tlbrecordS1.addrdesc.paddress.physicaladdress; 
      tlbrecordS2  <- tlbrecordS2; -- TranslationTableWalkLD(s1outputaddr, mva, iswrite, false, false);
      var s2fs1walk = false;
      CheckPermissionS2(tlbrecordS2.perms, mva, s1outputaddr, tlbrecordS2.level, iswrite, s2fs1walk);
      result <- CombineS1S2Desc(tlbrecordS1.addrdesc, tlbrecordS2.addrdesc)
      }
    else  result <- tlbrecordS1.addrdesc
  else
...
AddressDescriptor TranslateAddress(VA :: bits(32) , ispriv :: bool, iswrite :: bool, size :: nat) =
   match MemorySystemArchitecture()
    {
      case MemArch_VMSA => return  TranslateAddressV(VA, ispriv, iswrite, size)
      case MemArch_PMSA => #ASSERT ("PMSA not supported by l3 specs")
     }

unit AlignmentFaultP(address :: bits (32), iswrite :: bool) = nothing


-- earlear this was only an exception with the address, now it is a function
-- AlignmentFault() 
unit AlignmentFault(address :: bits (32), iswrite :: bool) =
  match MemorySystemArchitecture() 
  { 
   case MemArch_VMSA => AlignmentFaultV(address, iswrite, CurrentModeIsHyp() or CP15.HCR.TGE)
   case MemArch_PMSA => AlignmentFaultP(address, iswrite)
    }

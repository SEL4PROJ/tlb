...
definition 
  typ_det_tlb' :: "'a tlb_det_state'_scheme \<Rightarrow> tlb_entry set state_scheme"
where
...
lemma tlb_det_more [simp]:
  ...
  by (clarsimp simp: typ_det_tlb'_def state.defs)
...
instantiation tlb_det_state'_ext :: (type) mmu    
begin
definition   
 "(mmu_translate v :: ('a tlb_det_state'_scheme \<Rightarrow> _))
  = do {
     mem   <- read_state MEM;
...
thm entry_range_asid_tags_def

lemma
  "\<forall>v\<in>entry_range e. lookup tlb a v = Miss \<longrightarrow> (a,v) \<notin> asid_va_incon tlb"
   by (clarsimp simp: asid_va_incon_def)

lemma
  "asid_va_incon (tlb \<union> {e}) = {av. lookup tlb (fst av) (snd av) = Incon} \<union> {av\<in>entry_range_asid_tags e. \<exists>x. lookup tlb (fst av) (snd av) = Hit x \<and> x \<noteq> e }"
...
lemma
   "\<forall>av\<in>entry_range_asid_tags (pt_walk (ASID s) (MEM s) (TTBR0 s) va). lookup (tlb_det_set s) (fst av) (snd av) = Miss \<Longrightarrow>
        asid_va_incon (insert (pt_walk (ASID s) (MEM s) (TTBR0 s) va) (tlb_det_set s)) = asid_va_incon (tlb_det_set s)"
  apply (clarsimp simp: asid_va_incon_def)
  apply (clarsimp simp: asid_va_incon_def entry_range_asid_tags_def entry_range_def)
  apply (case_tac "pt_walk (ASID s) (MEM s) (TTBR0 s) va")
   apply clarsimp
oops

thm tlb_rel_absD

lemma mmu_translate_det_incon_tlb_rel_abs:
  "\<lbrakk> mmu_translate va s = (pa, s');  mmu_translate va t = (pa', t') ;
      consistent (typ_det_tlb' s) va ; no_faults (tlb_det_set' s) ; 
         (ASID t, addr_val va) \<notin> (tlb_incon_set t) ; tlb_rel_abs (typ_det_tlb' s) (typ_incon t) \<rbrakk> \<Longrightarrow> 
              tlb_rel_abs  (typ_det_tlb' s') (typ_incon t')"       
  ...
  apply (clarsimp simp: mmu_translate_tlb_det_state'_ext_def split_def Let_def)
  apply (case_tac "lookup (tlb_det_set' s) (ASID s) (addr_val va)" ; clarsimp)
    apply (clarsimp simp: Let_def)
    apply (clarsimp  split: split_if_asm)
     apply (clarsimp simp: raise'exception_def tlb_rel_abs_def state.defs split:split_if_asm)
    apply (rule conjI)
     apply (clarsimp simp:tlb_rel_abs_def state.defs)
    prefer 2
    apply (clarsimp simp: consistent0_def)
   prefer 2
   apply (subgoal_tac "\<not>is_fault x3")
    prefer 2
    apply (clarsimp simp: lookup_in_tlb no_faults_def)
   apply clarsimp
   apply (subgoal_tac "x3 = pt_walk (ASID s) (MEM s) (TTBR0 s) va ")
    prefer 2
    apply (clarsimp simp: consistent0_def)
   apply (clarsimp)
  apply (subgoal_tac "asid_va_incon (insert (pt_walk (ASID s) (MEM s) (TTBR0 s) va) (tlb_det_set' s)) = asid_va_incon (tlb_det_set' s)")
   apply clarsimp
  sorry



lemma mmu_translate_det_incon_pa:
  "\<lbrakk> mmu_translate va s = (pa, s');  mmu_translate va t = (pa', t') ;
      consistent (typ_det_tlb' s) va ; no_faults (tlb_det_set' s) ; 
         (ASID t, addr_val va) \<notin> (tlb_incon_set t) ; tlb_rel_abs (typ_det_tlb' s) (typ_incon t) \<rbrakk> \<Longrightarrow> 
             pa = pa'"       
...
lemma mmu_translate_det_incon_consistent:
  "\<lbrakk> mmu_translate va s = (pa, s');  mmu_translate va t = (pa', t') ;
      consistent (typ_det_tlb' s) va ; no_faults (tlb_det_set' s) ; 
         (ASID t, addr_val va) \<notin> (tlb_incon_set t) ; tlb_rel_abs (typ_det_tlb' s) (typ_incon t) \<rbrakk> \<Longrightarrow> 
                  consistent (typ_det_tlb' s') va"       
  apply (frule tlb_rel_absD , clarsimp)
  apply (clarsimp simp: mmu_translate_tlb_incon_state_ext_def Let_def)
  apply (clarsimp simp: tlb_rel_abs_def)
  apply (clarsimp simp: mmu_translate_tlb_det_state'_ext_def split_def Let_def)
...
lemma mmu_translate_det_incon_refine:
  "\<lbrakk> mmu_translate va s = (pa, s');  mmu_translate va t = (pa', t') ;
      consistent (typ_det_tlb' s) va ; no_faults (tlb_det_set' s) ; 
         (ASID t, addr_val va) \<notin> (tlb_incon_set t) ; tlb_rel_abs (typ_det_tlb' s) (typ_incon t) \<rbrakk> \<Longrightarrow> 
                  pa = pa' \<and>  consistent (typ_det_tlb' s') va \<and> tlb_rel_abs  (typ_det_tlb'  s') (typ_incon t')"   
  using mmu_translate_det_incon_tlb_rel_abs   mmu_translate_det_incon_pa mmu_translate_det_incon_consistent by fastforce


instantiation tlb_det_state'_ext :: (type) mem_op   
begin


definition   
  "(mmu_read  :: (vaddr  \<Rightarrow>  'a tlb_det_state'_scheme \<Rightarrow> bool list \<times>  'a tlb_det_state'_scheme))
   \<equiv>  \<lambda>va. do {
...
definition
 "(mmu_read_size  :: (vaddr \<times> nat \<Rightarrow>  'a tlb_det_state'_scheme \<Rightarrow> bool list \<times>  'a tlb_det_state'_scheme))
...
definition   
  "(mmu_write_size  :: (bool list \<times> vaddr \<times> nat \<Rightarrow> 'a tlb_det_state'_scheme \<Rightarrow> unit \<times> 'a tlb_det_state'_scheme))

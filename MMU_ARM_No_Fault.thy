theory MMU_ARM_No_Fault

imports ARM_Monadic_Ops
...
lemma lookup_refill:
  "lookup tlb asid (addr_val va) = Miss \<Longrightarrow>
   ...
   by (clarsimp simp: lookup_def entry_set_insert [where e="pt_walk asid m ttbr va"] split: split_if_asm)
...
lemma mmu_translate_det_flt_refine:
  ...
  apply (cases "lookup (tlb_det_set t) (ASID s) (addr_val va)")
    apply clarsimp
    apply (simp add: Let_def raise'exception_def typ_det_tlb_def state.defs tlb_rel_flt_def split: split_if_asm)
...
lemma mmu_translate_flt_refine_det:
  ...
  apply (cases "lookup (tlb_det_set t) (ASID s) (addr_val va)")
    apply clarsimp
    apply (simp add: Let_def raise'exception_def typ_det_tlb_def typ_tlb_def state.defs tlb_rel_flt_def split: split_if_asm)
  ...
  apply (clarsimp split:split_if_asm)
   apply (clarsimp simp: raise'exception_def tlb_rel_flt_def typ_tlb_def typ_det_tlb_def state.defs split: split_if_asm)
...
lemma  mmu_translate_sat_refine_non_det:        
  ...
  apply (cases "lookup (tlb_set s - tlb_evict (typ_tlb s)) (ASID s) (addr_val va)"; clarsimp)
   apply (clarsimp simp: consistent0_def Let_def tlb_rel_sat_def typ_sat_tlb_def typ_tlb_def state.defs
    ...
    apply (cases s, cases t, clarsimp simp:saturated_def) apply blast
   apply (cases s, cases t, clarsimp simp:saturated_def) apply blast
  apply (clarsimp split: split_if_asm simp:raise'exception_def tlb_rel_sat_def typ_sat_tlb_def typ_tlb_def state.defs)
    apply (cases s, cases t, clarsimp simp:saturated_def)
    using lookup_in_tlb apply blast+
done
...
thm mmu_translate_det_refine

thm mmu_translate_det_refine'

lemma tlb_rel_abs_consistent:
  "\<lbrakk>(ASID t, addr_val va) \<notin> (tlb_incon_set t) ;  tlb_rel_abs (typ_det_tlb s) (typ_incon t)  \<rbrakk>
            \<Longrightarrow> consistent (typ_det_tlb s) va" 
  apply (clarsimp simp: tlb_rel_abs_def)
  apply (clarsimp simp: asid_va_incon_def)
  apply (clarsimp simp: consistent0_def)
(*  so we have to do the saturation step *)
(* so we are saying here that lookup should be Hit  *)
...
lemma mmu_translate_sat_abs_refine':
  "\<lbrakk>mmu_translate va s = (pa, s'); mmu_translate va t = (pa', t'); saturated (typ_sat_tlb s);
    (ASID t, addr_val va) \<notin> tlb_incon_set t; tlb_rel_abs (typ_sat_tlb s) (typ_incon t)\<rbrakk>  \<Longrightarrow> 
                pa = pa' \<and> tlb_rel_abs (typ_sat_tlb s') (typ_incon t')"
   by (simp add: mmu_translate_sat_abs_refine mmu_translate_sat_abs_refine_pa)



lemma mmu_incon_hit_problem:
  "\<lbrakk>mmu_translate va s = (pa, s'); mmu_translate va t = (pa', t');
             (ASID t, addr_val va) \<notin> tlb_incon_set t; tlb_rel_abs (typ_det_tlb s) (typ_incon t)\<rbrakk>  \<Longrightarrow> pa = pa'"
  apply (frule tlb_rel_absD , clarsimp)
  apply (clarsimp simp: mmu_translate_tlb_incon_state_ext_def)
  ...
  apply (clarsimp split: "lookup_type.splits")
    ...
    apply (clarsimp split: split_if_asm)
    apply (clarsimp simp: raise'exception_def split:split_if_asm)
   apply (clarsimp simp: tlb_rel_abs_def asid_va_incon_def)
   apply force
...
find_theorems "lookup" "Hit" "is_fault"
end